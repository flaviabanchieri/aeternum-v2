...
class AeternumSupabaseUser extends BaseAuthUser {
  AeternumSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aeternumSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AeternumSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
